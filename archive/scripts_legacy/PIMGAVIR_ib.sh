...
###################configuration slurm##############################
# PIMGAVir - Infiniband Optimized Version
#SBATCH --job-name=PIMGAVir-IB
#SBATCH --output=pimgavir_ib.%A.out
#SBATCH --error=pimgavir_ib.%A.err
...
#Usage sbatch PIMGAVIR_ib.sh R1.fastq.gz R2.fastq.gz SampleName NumbOfCores ALL|[--read_based --ass_based --clust_based] [--filter]
#As an example: sbatch PIMGAVIR_ib.sh sample9_1.fastq.gz sample9_2.fastq.gz sample9 40 --read_based --filter
...
if [ -f "${HOME}/miniconda3/etc/profile.d/conda.sh" ]; then
    ...
    echo "For better performance, consider using PIMGAVIR_conda_ib.sh with conda environment"
fi
...
##Versioning
version="PIMGAVir V.2.0 -- 24.10.2025 (Infiniband Optimized)"
...
then
    ...
    printf "%b" "Usage PIMGAVIR_ib.sh R1.fastq.gz R2.fastq.gz SampleName NumbOfCores ALL|[--read_based --ass_based --clust_based] [--filter] \n" >&2
    exit 1
elif (( $# > $NumOfArgs+4 ))
then
...
if [ $5 == 'ALL' ];
	then
		...
		./taxonomy.sh $sequence_data $SampleName"_read-based-taxonomy" $JTrim _READ &
		PID_READ=$!
