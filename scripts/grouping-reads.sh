...
logfile="report/grouping-reads.log"
...
case $tag in
  	("--f")    
			...
			awk -v var=$data_type '{if ($2 != "0") 
				{
					cmd="echo " $2 " | taxonkit --data-dir ../DBs/NCBIRefSeq/ reformat -I 1 -f \"{f}\" "; 
					cmd | getline myinfo ; 
					close(cmd)
					split(myinfo,a," "); 
					#print "1: " a[1], "2: " a[2]; 
	...
	("--g")    
			...
			awk -v var=$data_type '{if ($2 != "0") 
				{
					cmd="echo " $2 " | taxonkit --data-dir ../DBs/NCBIRefSeq/ reformat -I 1 -f \"{g}\" "; 
					cmd | getline myinfo ; 
					close(cmd)
					split(myinfo,a," "); 
					#print "1: " a[1], "2: " a[2]; 
