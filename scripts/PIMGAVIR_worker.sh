...
###################configuration slurm##############################
# PIMGAVir Worker - Processes individual samples from array job
...
##Versioning
version="PIMGAVir V.2.1 -- 29.10.2025 (Worker - Conda Version)"
...
SCRATCH_DIRECTORY=/scratch/${USER}_${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}
...
# Define absolute path to databases on NAS (read-only access)
export PIMGAVIR_DBS_DIR="/projects/large/PIMGAVIR/pimgavir_dev/DBs"
echo "Database Configuration:"
echo "  Using databases from NAS: $PIMGAVIR_DBS_DIR"
echo "  This avoids copying ~170 GB of databases to scratch"
echo "  Databases accessed directly from NAS (read-only)"
echo ""

echo "Copying sample data to scratch directory..."
scp "$R1" ${SCRATCH_DIRECTORY}/pimgavir_dev/scripts/
scp "$R2" ${SCRATCH_DIRECTORY}/pimgavir_dev/scripts/
echo "Done"
echo ""
...
if [ -f "$NotrRNAReads" ];
	then
		...
		# Use 'source' instead of 'bash' to inherit conda environment
